...
# 1. Add export icon to imports
if "Download" not in text:
    text = text.replace("export {", "import { Download, Share2 } from 'lucide-react';\\nexport {")
    # Actually wait, icons are imported at the top. Let's just blindly add to top:
    text = "import { Download, UserPlus, Clock } from 'lucide-react';\n" + text

# Let's replace the ENTIRE CaseManager function using a regex.
# It's cleaner since the existing one is large.
    ...
    <div className="surface-card p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="space-y-3 lg:col-span-1 border-r border-[#E2E8F0] pr-4">
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded p-1.5 text-xs focus:outline-none focus:border-blue-500">
                ...
                {activeTab === 'overview' && (
                  <div className="space-y-4">
                    ...
                    <div className="surface-card p-4">
                      <h4 className="font-bold text-slate-800 mb-2">Attached Forensics ({activeCaseData.attached_analyses.length})</h4>
                ...
                {activeTab === 'timeline' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-3">
                      <h4 className="font-bold text-slate-800 flex items-center gap-2"><Clock className="w-4 h-4"/> Analyst Notes</h4>
...
m = re.search(r'export function CaseManager.*?}(?=\nexport|\Z)', text, flags=re.DOTALL)
if m:
    text = text[:m.start()] + soc_case_manager + text[m.end():]
    with open('frontend/src/components/Views.jsx', 'w', encoding='utf-8') as f:
        f.write(text)
else:
    print("Failed to replace CaseManager.")
